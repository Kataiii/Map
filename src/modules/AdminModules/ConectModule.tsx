...
const ConectModule = () => {
    ...
    const getLocalityid = (departamentVTB: IDepartamentVTB): number => {
        const address = departamentVTB.address;
        let parts = address.split(', ');
        let locality = localities?.find(locality => locality.name == parts[2]);
        console.log('locality');
        console.log(locality);
        if(locality == undefined){
            let state = states?.find(state => state.name == parts[1]);
            console.log('state');
            console.log(state);
            if(state == undefined){
                createState([{id:0, name: parts[1], countryId: 1} as IState]);
            }
            state = states?.find(state => state.name == parts[1]);
            createLocality([{id:0, name: parts[2], stateId: state?.id} as ILocality]);
    ...
    const createDays = (days: IWorkDayVTB[]): IWorkDay| undefined => {
        ...
        return workDays?.find(item => item.id == idWorkDay);
    }
    ...
    }, [isLoadingWorkDays, isLoadingState, isLoandingLocality, isLoadingMy])

    const expandDays = (days: IWorkDayVTB[]): IWorkDay| undefined => {
        if(days.length == 7){
    ...
    const showFile = async (e: any) => {
        ...
        reader.onload = async (e) => { 
          ...
          if(text !== undefined && text != null){
            ...
            setIsLoadingMy(true);
            let departamentsGen = obj.map(item => ({
                id: 0,
                ...
                workDaysUrId: expandDays(item.openHours),
                workDaysFizId: expandDays(item.openHoursIndividual),
                description: ''
            } as IDepartament))
